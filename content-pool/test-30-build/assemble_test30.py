#!/usr/bin/env python3
"""
Assemble Test 30 into insert-ready JSON.
...
Two structural rules are enforced by construction, then re-checked:
  ...
  * Per-question difficulty follows the module. This is the Test 1/2
    ...
    their Question Bank difficulty badges and filters are wrong. Test 30 does
    not repeat that.

Nothing here is reused: the Math is authored in math_test30.py and verified
independently by verify_math_test30.py, and every R&W passage in
rw_test30.py was screened against the 809-passage corpus in
../rw_authored_corpus.json before it was drafted.

Run:  python3 assemble_test30.py
Out:  test30.json
"""
import json
import os
...
from math_test30 import (MODULE_1 as MATH_M1, MODULE_2_EASY as MATH_M2E,      # noqa: E402
...
def load_rw_pool():
    ...
    with open(os.path.join(HERE, "rw_test30_balanced.json")) as fh:
        rows = json.load(fh)
    pool = defaultdict(list)
    for q in rows:
        item = dict(q)
        item["_ref"] = f"AUTHORED-T30:{q['num']}"
...
def build_math():
    def conv(items, mod_key):
        ...
        for q in items:
            ...
            row = {
                "_ref": f"AUTHORED/T30-{mod_key}:{q['n']}",
...
if __name__ == "__main__":
    ...
    ok = report(test)
    with open(os.path.join(HERE, "test30.json"), "w") as fh:
        json.dump(test, fh, indent=1, ensure_ascii=False)
    print(f"\nwrote test30.json  —  {'OK' if ok else 'WITH FAILURES'}")
