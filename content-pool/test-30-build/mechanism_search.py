...
MECHANISMS = {
 ...
 "H1-06": ("read a linear rule off a 3-row table of (input, output)",
           r"(?i)\blinear function of\b.{0,200}Which equation"),
 ...
 "H1-08": ("difference of two squared binomials expands to a single term",
           r"\(\s*[^()]{0,20}\)\^\{?2\}?\s*-\s*\\?\(?\s*[^()]{0,20}\)\^\{?2\}?"),
 "H1-09": ("factor a quadratic over a quadratic and cancel the common factor",
           r"\\frac\{[^{}]*x\^\{?2\}?[^{}]*\}\{[^{}]*x\^\{?2\}?[^{}]*\}"),
 "H1-10": ("minimum value of an upward parabola given in standard form",
           r"(?i)\b(?:least|minimum|lowest) (?:value|temperature|height|cost|number)\b"),
 ...
 "H1-18": ("two-way table: one cell total over the grand total, as a percent",
           r"(?i)\bwhat percent(?:age)? of (?:all|the) \w+ (?:in the table|listed|shown)\b"),
 "H1-19": ("cone volume given, solve for a dimension",
           r"(?i)\bcone\b"),
 "H1-20": ("volume of a rectangular box divided by the volume of one container",
           r"(?i)\bhow many (?:bags|loads|barrows|buckets|sacks|tins|cans|containers)\b.{0,200}\b(?:cubic|volume)\b"),
 "H1-21": ("isosceles apex angle -> base angle -> bisected base angle",
           r"(?i)\bbisects?\b.{0,120}\bangle\b|\bangle bisector\b"),
 "H1-22": ("right triangle: a trig ratio plus one length, find an area",
           r"(?i)\b(?:\\tan|\\sin|\\cos)\s*[A-Z]\b.{0,200}\barea\b"),
 ...
 "H2E-05": ("evaluate f(x)=mx+b at a stated x",
            r"(?i)f\(\w\)\s*=\s*\d*\w?\s*[-+]\s*\d+.{0,200}\bwhat is\b"),
 ...
 "H2E-08": ("distribute over a bracket and collect like terms",
            r"(?i)\d\s*\(\s*\d?\w\s*[-+]\s*\d+\s*\)\s*[-+]\s*\d\w"),
 ...
 "H2E-11": ("evaluate a quadratic model of the form constant minus x^2/k",
            r"(?i)\d+\s*-\s*\\frac\{\w\^\{?2\}?\}\{\d+\}|\d+\s*-\s*\\frac\{\w\^2\}"),
 "H2E-12": ("evaluate a downward parabola -x^2+bx at a stated x",
            r"-\s*\w\^\{?2\}?\s*\+\s*\d+\s*\w"),
 "H2E-13": ("solve x^2 + c = k for the positive root",
            r"(?i)\bpositive value of\b|\bx is positive\b.{0,120}\^\{?2\}?"),
 ...
 "H2E-16": ("median of five listed values",
            r"(?i)\bmedian\b.{0,200}\b(?:five|5)\b|\bfive\b.{0,200}\bmedian\b"),
 "H2E-17": ("probability of one category from a total with 'the rest'",
            r"(?i)\bat random\b.{0,220}\bprobability\b"),
 ...
 "H2E-19": ("volume of a rectangular box",
            r"(?i)\bvolume\b.{0,200}\brectangular\b|\brectangular\b.{0,200}\bvolume\b"),
 "H2E-20": ("perimeter of a rectangle from length and width",
            r"(?i)\bperimeter\b"),
 "H2E-21": ("third angle of a triangle from the other two",
            r"(?i)\bthird angle\b"),
 "H2E-22": ("a trig ratio read directly off two given side lengths",
            r"(?i)\bwhat is the value of \\?\(?\\(?:sin|cos|tan)\b"),

 # ---- Module 2 (Hard) ------------------------------------------------------
 "H2H-01": ("absolute value equal to a linear expression, sum the roots",
            r"(?i)\|.{0,20}\|\s*=|\\left\|"),
 ...
 "H2H-05": ("slope from two points, apply to a parallel line through a third point",
            r"(?i)\bparallel\b.{0,200}\bpasses through\b"),
 ...
 "H2H-10": ("set a discriminant to zero to find a parameter",
            r"(?i)\bexactly one (?:real )?solution\b"),
 "H2H-11": ("equal outputs fix the axis of symmetry; the minimum then fixes c",
            r"(?i)f\(-?\d+\)\s*=\s*f\(-?\d+\)"),
 "H2H-12": ("radical equal to a linear expression, discard the extraneous root",
            r"\\sqrt\{[^{}]*\}\s*=\s*\w\s*[-+]\s*\d"),
 ...
 "H2H-19": ("cylinder with height tied to its diameter; one pi-quantity given",
            r"(?i)\bheight is (?:equal to|twice|the same as)\b.{0,60}\b(?:diameter|radius)\b"),
