#!/usr/bin/env python3
"""
Assemble Test 27 into insert-ready JSON.
...
Two structural rules are enforced by construction, then re-checked:
  ...
  * Per-question difficulty follows the module. This is the Test 1/2
    ...
    their Question Bank difficulty badges and filters are wrong. Test 27 does
    not repeat that.

Nothing here is reused: the Math is authored in math_test27.py and verified
independently by verify_math_test27.py, and every R&W passage in
rw_test27.py was screened against the 1,295-passage corpus in
../rw_authored_corpus.json before it was drafted.

Run:  python3 assemble_test27.py
Out:  test27.json
"""
import json
import os
...
from math_test27 import (MODULE_1 as MATH_M1, MODULE_2_EASY as MATH_M2E,      # noqa: E402
...
def load_rw_pool():
    ...
    with open(os.path.join(HERE, "rw_test27_balanced.json")) as fh:
        rows = json.load(fh)
    pool = defaultdict(list)
    for q in rows:
        item = dict(q)
        item["_ref"] = f"AUTHORED-T27:{q['num']}"
...
def build_math():
    def conv(items, mod_key):
        ...
        for q in items:
            ...
            row = {
                "_ref": f"AUTHORED/T27-{mod_key}:{q['n']}",
...
if __name__ == "__main__":
    ...
    ok = report(test)
    with open(os.path.join(HERE, "test27.json"), "w") as fh:
        json.dump(test, fh, indent=1, ensure_ascii=False)
    print(f"\nwrote test27.json  —  {'OK' if ok else 'WITH FAILURES'}")
