#!/usr/bin/env python3
"""
Assemble Test 7 into insert-ready JSON.
...
Two structural rules are enforced by construction, then re-checked:
  ...
  * Per-question difficulty follows the module. This is the Test 1/2
    ...
    their Question Bank difficulty badges and filters are wrong. Test 7 does
    not repeat that.
...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "test-6-7-build"))

from math_test7 import MODULE_1 as MATH_M1, MODULE_2_HARD as MATH_M2H   # noqa: E402
from math_m2easy import TEST7 as MATH_M2E                               # noqa: E402
import rw_octusb_m1, rw_octintb_m1, rw_octusc_m1, rw_octintb_m2         # noqa: E402
import rw_octusb_m2, rw_octusc_m2, rw_auguse, rw_authored               # noqa: E402
import rw_test7_extra                                                   # noqa: E402

HERE = os.path.dirname(os.path.abspath(__file__))
SIX = os.path.join(HERE, "..", "test-6-7-build")
...
def load_rw_pool():
    """Every R&W item Test 6 did not consume, keyed by block."""
    used = set()
    with open(os.path.join(SIX, "test6.json")) as fh:
        six = json.load(fh)
    for mod in ("RW_M1", "RW_M2E", "RW_M2H"):
        for q in six[mod]:
            used.add(q["_ref"])

    pool = defaultdict(list)
    modules = [rw_octusb_m1, rw_octintb_m1, rw_octusc_m1, rw_octintb_m2,
               rw_octusb_m2, rw_octusc_m2, rw_auguse, rw_authored,
               rw_test7_extra]
    for m in modules:
        mod_name = getattr(m, "MODULE", "")
        for q in m.QUESTIONS:
            ref = f"{m.SOURCE}/{mod_name}:{q['num']}" if mod_name else f"{m.SOURCE}:{q['num']}"
            if ref in used:
                continue
            item = dict(q)
            item["_ref"] = ref
...
if __name__ == "__main__":
    ...
    ok = report(test)
    with open(os.path.join(HERE, "test7.json"), "w") as fh:
        json.dump(test, fh, indent=1, ensure_ascii=False)
    print(f"\nwrote test7.json  —  {'OK' if ok else 'WITH FAILURES'}")
