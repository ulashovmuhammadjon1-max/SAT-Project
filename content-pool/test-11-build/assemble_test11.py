#!/usr/bin/env python3
"""
Assemble Test 11 into insert-ready JSON.
...
Two structural rules are enforced by construction, then re-checked:
  ...
  * Per-question difficulty follows the module. This is the Test 1/2
    ...
    their Question Bank difficulty badges and filters are wrong. Test 11 does
    not repeat that.
...
Run:  python3 assemble_test11.py
Out:  test11.json
"""
import json
import os
...
from math_test11 import (MODULE_1 as MATH_M1, MODULE_2_EASY as MATH_M2E,      # noqa: E402
...
def load_rw_pool():
    ...
    with open(os.path.join(HERE, "rw_test11_balanced.json")) as fh:
        rows = json.load(fh)
    pool = defaultdict(list)
    for q in rows:
        item = dict(q)
        item["_ref"] = f"AUTHORED-T11:{q['num']}"
...
if __name__ == "__main__":
    ...
    ok = report(test)
    with open(os.path.join(HERE, "test11.json"), "w") as fh:
        json.dump(test, fh, indent=1, ensure_ascii=False)
    print(f"\nwrote test11.json  —  {'OK' if ok else 'WITH FAILURES'}")
