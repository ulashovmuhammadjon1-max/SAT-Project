#!/usr/bin/env python3
"""
Assemble Test 17 into insert-ready JSON.
...
Two structural rules are enforced by construction, then re-checked:
  ...
  * Per-question difficulty follows the module. This is the Test 1/2
    ...
    their Question Bank difficulty badges and filters are wrong. Test 17 does
    not repeat that.

Nothing here is reused: the Math is authored in math_test17.py and verified
independently by verify_math_test17.py, and every R&W passage in
rw_test17.py was screened against the 809-passage corpus in
../rw_authored_corpus.json before it was drafted.

Run:  python3 assemble_test17.py
Out:  test17.json
"""
import json
import os
...
from math_test17 import (MODULE_1 as MATH_M1, MODULE_2_EASY as MATH_M2E,      # noqa: E402
...
def load_rw_pool():
    ...
    with open(os.path.join(HERE, "rw_test17_balanced.json")) as fh:
        rows = json.load(fh)
    pool = defaultdict(list)
    for q in rows:
        item = dict(q)
        item["_ref"] = f"AUTHORED-T17:{q['num']}"
...
def build_math():
    def conv(items, mod_key):
        ...
        for q in items:
            ...
            row = {
                "_ref": f"AUTHORED/T17-{mod_key}:{q['n']}",
...
if __name__ == "__main__":
    ...
    ok = report(test)
    with open(os.path.join(HERE, "test17.json"), "w") as fh:
        json.dump(test, fh, indent=1, ensure_ascii=False)
    print(f"\nwrote test17.json  —  {'OK' if ok else 'WITH FAILURES'}")
