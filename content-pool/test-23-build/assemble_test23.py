#!/usr/bin/env python3
"""
Assemble Test 23 into insert-ready JSON.
...
Two structural rules are enforced by construction, then re-checked:
  ...
  * Per-question difficulty follows the module. This is the Test 1/2
    ...
    their Question Bank difficulty badges and filters are wrong. Test 23 does
    not repeat that.

Nothing here is reused: the Math is authored in math_test23.py and verified
independently by verify_math_test23.py, and every R&W passage in
rw_test23.py was screened against the 1,295-passage corpus in
../rw_authored_corpus.json before it was drafted.

Run:  python3 assemble_test23.py
Out:  test23.json
"""
import json
import os
...
from math_test23 import (MODULE_1 as MATH_M1, MODULE_2_EASY as MATH_M2E,      # noqa: E402
...
# 15 reading + 12 writing, so the writing block opens at question 16. Unlike
# Test 17 this test carries a Cross-Text Connections item in every module: the
# block is optional in the mandated sequence but it is a real SAT domain, and
# the reading side has room for it at 15.
QUOTA = {
    ...
    "Boundaries": 3, "Form, Structure, and Sense": 3, "Transitions": 3,
...
def load_rw_pool():
    ...
    with open(os.path.join(HERE, "rw_test23_balanced.json")) as fh:
        rows = json.load(fh)
    pool = defaultdict(list)
    for q in rows:
        item = dict(q)
        item["_ref"] = f"AUTHORED-T23:{q['num']}"
...
def build_math():
    def conv(items, mod_key):
        ...
        for q in items:
            ...
            row = {
                "_ref": f"AUTHORED/T23-{mod_key}:{q['n']}",
...
if __name__ == "__main__":
    ...
    ok = report(test)
    with open(os.path.join(HERE, "test23.json"), "w") as fh:
        json.dump(test, fh, indent=1, ensure_ascii=False)
    print(f"\nwrote test23.json  —  {'OK' if ok else 'WITH FAILURES'}")
