#!/usr/bin/env python3
"""
Assemble Test 12 into insert-ready JSON.
...
Two structural rules are enforced by construction, then re-checked:
  ...
  * Per-question difficulty follows the module. This is the Test 1/2
    ...
    their Question Bank difficulty badges and filters are wrong. Test 12 does
    not repeat that.
...
Run:  python3 assemble_test12.py
Out:  test12.json
"""
import json
import os
...
from math_test12 import (MODULE_1 as MATH_M1, MODULE_2_EASY as MATH_M2E,      # noqa: E402
...
def load_rw_pool():
    ...
    with open(os.path.join(HERE, "rw_test12_balanced.json")) as fh:
        rows = json.load(fh)
    pool = defaultdict(list)
    for q in rows:
        item = dict(q)
        item["_ref"] = f"AUTHORED-T12:{q['num']}"
...
if __name__ == "__main__":
    ...
    ok = report(test)
    with open(os.path.join(HERE, "test12.json"), "w") as fh:
        json.dump(test, fh, indent=1, ensure_ascii=False)
    print(f"\nwrote test12.json  —  {'OK' if ok else 'WITH FAILURES'}")
