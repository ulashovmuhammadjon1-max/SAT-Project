#!/usr/bin/env python3
"""
Assemble Test 16 into insert-ready JSON.
...
def load_rw_pool():
    ...
    with open(os.path.join(HERE, "rw_test16_balanced.json")) as fh:
        rows = json.load(fh)
    pool = defaultdict(list)
    for q in rows:
        item = dict(q)
        item["_ref"] = f"AUTHORED-T16:{q['num']}"
...
if __name__ == "__main__":
    ...
    ok = report(test)
    with open(os.path.join(HERE, "test16.json"), "w") as fh:
        json.dump(test, fh, indent=1, ensure_ascii=False)
    print(f"\nwrote test16.json  —  {'OK' if ok else 'WITH FAILURES'}")
