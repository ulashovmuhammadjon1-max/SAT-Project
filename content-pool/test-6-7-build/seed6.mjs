 ...
 *
 *   node seed_attempt.mjs "Test 6" READING_WRITING
 */
import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();
const title = process.argv[2] ?? "Test 6";
...
const mod = await prisma.module.findFirst({
  where: { testId: test.id, subject, order: 1 },
});
if (!mod) throw new Error(`no ${subject} Module 1 on "${title}"`);
