#!/usr/bin/env python3
"""
Assemble Test 29 into insert-ready JSON.
...
Two structural rules are enforced by construction, then re-checked:
  ...
  * Per-question difficulty follows the module. This is the Test 1/2
    ...
    their Question Bank difficulty badges and filters are wrong. Test 29 does
    not repeat that.

Nothing here is reused: the Math is authored in math_test29.py and verified
independently by verify_math_test29.py, and every R&W passage in
rw_test29.py was screened by screen_topics.py against the 1,295-passage corpus in
../rw_authored_corpus.json AND against every other passage in this build, the
second check being the one a single narrow territory actually needs.

Run:  python3 assemble_test29.py
Out:  test29.json
"""
import json
import os
...
from math_test29 import (MODULE_1 as MATH_M1, MODULE_2_EASY as MATH_M2E,      # noqa: E402
...
# Sums to the pool exactly: 3 x each of the above is the count authored in
# rw_test29.py, so `shortfall` must come back empty and no block is dealt from a
# module that has run dry. Asserted after build_rw().
...
def load_rw_pool():
    ...
    with open(os.path.join(HERE, "rw_test29_balanced.json")) as fh:
        rows = json.load(fh)
    pool = defaultdict(list)
    for q in rows:
        item = dict(q)
        item["_ref"] = f"AUTHORED-T29:{q['num']}"
...
def build_math():
    def conv(items, mod_key):
        ...
        for q in items:
            ...
            row = {
                "_ref": f"AUTHORED/T29-{mod_key}:{q['n']}",
...
if __name__ == "__main__":
    ...
    assert not shortfall, f"quota exceeds the authored pool: {dict(shortfall)}"
    leftover = {b: len(v) for b, v in pool.items() if v}
    assert not leftover, f"authored items never dealt: {leftover}"

    test = {mod: convert_rw(rw[mod], mod) for mod in MODULES}
    test.update(build_math())

    ok = report(test)
    with open(os.path.join(HERE, "test29.json"), "w") as fh:
        json.dump(test, fh, indent=1, ensure_ascii=False)
    print(f"\nwrote test29.json  —  {'OK' if ok else 'WITH FAILURES'}")
