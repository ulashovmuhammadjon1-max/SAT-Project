...
QUESTIONS = [
 ...
 dict(q="The presence of ribosomes in every known form of life and the endosymbiotic origin of the mitochondrion illustrate two different evolutionary relationships. Which description of the pair is correct?",
   choices=[
     "Ribosomes reflect descent of all life from a shared ancestor, while the mitochondrion reflects the merger of two separate lineages",
     "Both reflect descent of all life from a shared ancestor and neither involves more than one lineage",
     "Both reflect the merger of two separate lineages within a single cell",
     "Ribosomes reflect the merger of two lineages, while the mitochondrion reflects descent from a shared ancestor",
     "Neither has any bearing on evolutionary relationships between organisms"],
   ans=0,
   why="EK 2.1.A.1 reads the universality of ribosomes as reflecting the common ancestry of all known life, a single line of descent. EK 2.10.A.1 describes something else entirely, one lineage of cells taken up into another, which is a merger rather than a branching."),

 dict(q="Which question about cell compartmentalization can be settled by observing cells, rather than by a judgement that observation cannot decide?",
   choices=[
     "Do prokaryotic cells contain regions in which particular functions are carried out?",
     "Should a prokaryotic cell be regarded as a simpler organism than a eukaryotic one?",
     "Is the mitochondrion the most impressive organelle in a eukaryotic cell?",
     "Would a cell be better off if it had never acquired an endosymbiont?",
     "Does a eukaryotic cell deserve more study than a prokaryotic cell?"],
   ans=0,
   why="Skill 3.A asks for a testable question. Only the keyed question can be answered by looking at cells, and EK 2.10.A.2 is the statement it would test; the other four ask what is simpler, more impressive, better or more deserving, which no observation decides."),
