...
QUESTIONS = [
 ...
 dict(q="Which development described by the framework moves authority away from the constituent units of a federation and toward its national executive?",
   choices=[
     "presidential envoys placed over a set of federal districts, alongside a rule letting regional legislatures pick a governor only from a list the president approves",
     "the establishment of religious courts in one region of a federal state under that state's system of federalism",
     "constitutional reforms devolving power to multiple parliaments",
     "a supreme court acquiring the function of ruling on disputes about devolved powers",
     "a senate exercising a long-standing constitutional power to approve intervention in the affairs of a constituent unit"], ans=0,
   why="EK DEM-2.B.5.c presents both measures as reasserting federal power under the Russian president, and EK PAU-2.A.2 allows the degree of centralization to shift inside a federal structure. The rejected options either devolve authority, adjudicate its boundaries, or exercise a power the constitution already allocated rather than relocating it."),
