...
QUESTIONS = [
 ...
 dict(q="A gene is normally inducible: it is expressed only when a particular signal is present. A cell is found that lacks the regulatory protein that normally keeps this gene switched off. What is expected of the gene in that cell?",
   choices=[
     "It is expressed even without the signal, because the interaction that kept it off is missing",
     "It is expressed only when the signal is present, exactly as in other cells",
     "It is never expressed, because a gene needs a regulatory protein in order to be transcribed",
     "It becomes constitutively expressed in every cell of the organism, not only in this one",
     "It is removed from the chromosome, since it can no longer be regulated"], ans=0,
   why="EK 6.5.A.1 states that regulatory sequences are stretches of DNA that interact with regulatory proteins to control transcription, and that some genes are constitutively expressed while others are inducible. The control over an inducible gene comes from that interaction, so a cell lacking the protein loses the switch rather than losing transcription; the change concerns this cell, since EK 6.5.A.3 makes expression a property of a cell as well as of an organism."),
