...
assert vol((x - x**3)**2, 0, 1) == sp.Rational(8, 105)
assert vol(2 * (x - x**3)**2, 0, 1) == sp.Rational(16, 105) and num(8) == sp.Rational(16, 105)
...
assert vol((x**2)**2 / 2, 0, 2) == sp.Rational(16, 5) and num(13) == sp.Rational(16, 5)
...
assert sp.integrate(9 - x**2, (x, -3, 3)) == 36
assert vol(5 * (9 - x**2), -3, 3) == 180 and num(17) == 180
assert key(18) == "cubic centimeters"
assert key(19).startswith("pi belongs to circular cross sections")
assert sp.integrate(sp.sqrt(x), (x, 0, 9)) == 18
assert vol(2 * sp.sqrt(x), 0, 9) == 36 and num(20) == 36
