...
QUESTIONS = [
 ...
 dict(q="Breeders working from one wild species produce several varieties, each uniform in the trait its breeders emphasized but very different from the others. In what two ways has the framework's statement about affecting variation been borne out?",
   choices=[
     "The variation within each variety has been narrowed while the range of forms across the species as a whole has been widened",
     "The variation within each variety has been widened while the range of forms across the species as a whole has been narrowed",
     "The variation has been narrowed both within each variety and across the species as a whole",
     "The variation has been widened both within each variety and across the species as a whole",
     "The variation has been left unchanged, since the varieties all belong to one species"], ans=0,
   why="EK 7.3.A.1's verb is affect, which does not fix a direction, and this module's data show both directions at once: one table records the range of a trait narrowing across twenty generations of breeding within a line, and another records three varieties each exceeding the wild form on a different trait. Narrowing within a line and widening across the species are not in conflict, because they are variation measured at two different levels."),
 ...
 dict(q="A student describes a group of people who choose partners according to a trait they admire as an example of artificial selection in the framework's sense. Why does this not fit the framework's statement?",
   choices=[
     "The framework's statement is that humans affect variation in other species, and the people described are acting on their own",
     "The framework's statement covers only traits that can be measured in a laboratory",
     "The framework's statement covers only traits that show no variation",
     "The framework's statement requires that the selected individuals be reduced in number",
     "The framework's statement applies only to populations that are not changing"], ans=0,
   why="EK 7.3.A.1 states that through artificial selection humans affect variation in OTHER species, which is the clause the example falls outside. The framework attaches no requirement about laboratories, about how variable the trait is, or about the size or stability of the population."),
