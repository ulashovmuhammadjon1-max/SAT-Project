...
QUESTIONS = [
 ...
 dict(q="Among the rules the framework credits with facilitating a transition away from one-party dominance is the privatization of state-owned corporations. The reason the framework gives for including it is that privatization",
   choices=[
     "decreases patronage",
     "raises the threshold parties must clear to enter the legislature",
     "transfers the certification of election results to the courts",
     "requires the president to win an absolute majority",
     "guarantees seats in the legislature to opposition parties"], ans=0,
   why="EK PAU-4.A.4 states the purpose in the same clause, listing privatizing state-owned corporations to decrease patronage among the rules facilitating Mexico's transition away from one-party dominance. A party that can no longer distribute posts and contracts through state firms loses one of the resources that sustained its dominance."),
