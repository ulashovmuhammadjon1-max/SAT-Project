#!/usr/bin/env python3
"""
Assemble Test 8 into insert-ready JSON.
...
Two structural rules are enforced by construction, then re-checked:
  ...
  * Per-question difficulty follows the module. This is the Test 1/2
    ...
    their Question Bank difficulty badges and filters are wrong. Test 8 does
    not repeat that.
...
from math_test8 import (MODULE_1 as MATH_M1, MODULE_2_EASY as MATH_M2E,      # noqa: E402
...
def load_rw_pool():
    ...
    with open(os.path.join(HERE, "rw_test8_balanced.json")) as fh:
        rows = json.load(fh)
    pool = defaultdict(list)
    for q in rows:
        item = dict(q)
        item["_ref"] = f"AUTHORED-T8:{q['num']}"
...
if __name__ == "__main__":
    ...
    ok = report(test)
    with open(os.path.join(HERE, "test8.json"), "w") as fh:
        json.dump(test, fh, indent=1, ensure_ascii=False)
    print(f"\nwrote test8.json  —  {'OK' if ok else 'WITH FAILURES'}")
