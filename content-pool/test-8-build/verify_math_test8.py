#!/usr/bin/env python3
"""
Verify the 44 originally-authored Math questions for Test 8.
...
Run:  DATABASE_URL='postgresql://...' python3 verify_math_test8.py
...
from math_test8 import MODULE_1, MODULE_2_EASY, MODULE_2_HARD, ALL
...
# ---------------------------------------------------------------- derivations
SQ = lambda n: symbols(n, positive=True)


def a2h07():
    roots = solve(Eq(3 * x + 13, (x + 1) ** 2), x)
    return [r for r in roots if sqrt(3 * r + 13) == r + 1][0]


DERIVE = {
 "A1-01": lambda: solve(Eq(5 * (x - 3) - 2 * (x + 4), 13), x)[0],
 "A1-02": lambda: (lambda s_: 2 * s_[x] + s_[y])(solve([Eq(3 * x + 4 * y, 25), Eq(5 * x - 2 * y, 7)], [x, y])),
 "A1-03": lambda: solve(Eq(7 + Rational(-5 - 7, 4 - (-2)) * (x - (-2)), 0), x)[0],
 "A1-04": lambda: max(i for i in range(60) if 1850 + 45 * i <= 3200),
 "A1-05": lambda: solve(Eq(k / 6, Rational(7, 14)), k)[0],
 "A1-06": lambda: 8 - Rational(20 - 8, 7 - 3) * 3,
 "A1-07": lambda: solve(Eq(2 * x + 3 * (x - 8), 156), x)[0],
 "A1-08": lambda: simplify((3 * x - 2) ** 2 - (x + 5) ** 2),
 "A1-09": lambda: -2 * 4 ** 2 + 16 * 4 - 27,
 "A1-10": lambda: solve(Eq(4 ** (2 * x - 1), 64), x)[0],
 "A1-11": lambda: max(solve(Eq(2 * x ** 2 - 9 * x - 5, 0), x)) - min(solve(Eq(2 * x ** 2 - 9 * x - 5, 0), x)),
 "A1-12": lambda: simplify((x ** 2 - x - 12) / (x + 3)),
 "A1-13": lambda: max(solve(Eq(Rational(-49, 10) * x ** 2 + Rational(294, 10) * x, 0), x)),
 "A1-14": lambda: "half every 12",
 "A1-15": lambda: Rational(174, 10) * Rational(25, 10) + 12,
 "A1-16": lambda: Rational(728 * 100, 112),
 "A1-17": lambda: Rational(6 * 3 + 7 * 5 + 8 * 9 + 9 * 6 + 10 * 2, 25),
 "A1-18": lambda: Rational(6, 15) * Rational(5, 14),
 "A1-19": lambda: 6 * sqrt(Rational(125, 45)),
 "A1-20": lambda: Rational(96, 3),
 "A1-21": lambda: abs([r for r in solve(Eq(discriminant(Poly(x ** 2 + k * x + 36, x)), 0), k) if r < 0][0]),
 "A1-22": lambda: sqrt(24 ** 2 + 7 ** 2),

 "A2E-01": lambda: solve(Eq(x + 9, 23), x)[0],
 "A2E-02": lambda: solve(Eq(6 * y, 54), y)[0],
 "A2E-03": lambda: "C=3p",
 "A2E-04": lambda: 5 + 2 * 4,
 "A2E-05": lambda: 4,
 "A2E-06": lambda: Rational(48, 6),
 "A2E-07": lambda: 7,
 "A2E-08": lambda: simplify(4 * (x + 3)),
 "A2E-09": lambda: simplify(SQ("x") ** 2 * SQ("x") ** 2 * SQ("x") ** 3),
 "A2E-10": lambda: 5 ** 2 - 6,
 "A2E-11": lambda: sqrt(S(49)),
 "A2E-12": lambda: sqrt(81) + sqrt(16),
 "A2E-13": lambda: simplify(10 * SQ("x") ** 4 / (2 * SQ("x"))),
 "A2E-14": lambda: Rational(30, 100) * 260,
 "A2E-15": lambda: Rational(240, 90) * 6,
 "A2E-16": lambda: Rational(12 + 15 + 18 + 19 + 26, 5),
 "A2E-17": lambda: 410 - 220,
 "A2E-18": lambda: Rational(4, 10),
 "A2E-19": lambda: 180 - 55 - 65,
 "A2E-20": lambda: 14 * 9,
 "A2E-21": lambda: 14,
 "A2E-22": lambda: sqrt(36 + 64),

 "A2H-01": lambda: "2p-3q=0",
 "A2H-02": lambda: solve(Eq(-1, Rational(-3, 4) * 8 + x), x)[0],
 "A2H-03": lambda: 38 + min(e for e in range(40) if 16 * 38 + 24 * e >= 704),
 "A2H-04": lambda: len(solve(Eq(discriminant(Poly(3 * x ** 2 + b * x + 12, x)), 0), b)),
 "A2H-05": lambda: solve(Eq(k * (2 - 4) ** 2 + 7, 19), k)[0] * (8 - 4) ** 2 + 7,
 "A2H-06": lambda: 11 ** 2 + 2,
 "A2H-07": a2h07,
 "A2H-08": lambda: simplify(((Rational(8, 27) * SQ("x") ** 9) ** Rational(1, 3)) ** 2),
 "A2H-09": lambda: solve(Eq(k * (Rational(3, 2) ** 2 - 1), 25), k)[0],
 "A2H-10": lambda: 2 * sqrt(-16 + 25 + 16),
 "A2H-11": lambda: solve(Eq(Rational(15, 100) * (20 - v) + Rational(40, 100) * v, Rational(24, 100) * 20), v)[0],
 "A2H-12": lambda: Rational(12 * 31 - 18 - 24, 10),
 "A2H-13": lambda: Rational(156, 258),
 "A2H-14": lambda: 100 - Rational(85, 100) * Rational(80, 100) * 100,
 "A2H-15": lambda: 26 * Rational(150, 10) - 25 * Rational(148, 10),
 "A2H-16": lambda: Rational(15 * 20, sqrt(15 ** 2 + 20 ** 2)),
 "A2H-17": lambda: (Rational(3, 4) * 4 ** 2 * 18) ** Rational(1, 3),
 "A2H-18": lambda: sqrt(S(289) - 64) / 17,
 "A2H-19": lambda: 5 + Rational(18, 4) * (9 - 1),
 "A2H-20": lambda: Rational(8, 3),
 "A2H-21": lambda: 260 - solve(Eq(12 * x + 7 * (260 - x), 2470), x)[0],
 "A2H-22": lambda: solve(Eq(Rational(1, 3) * pi * 36 * x, 96 * pi), x)[0],
}
...
for q in ALL:
    ...
    ok = False
    for loc in ({}, {n: symbols(n, positive=True) for n in ("a", "b", "x", "y")}):
        try:
